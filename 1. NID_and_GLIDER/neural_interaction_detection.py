...
def interactions_output(
    ...
    **kwargs
    ...
    for i, inter_tuple in enumerate(inters):
        inter, strength = inter_tuple
        feature_inter = list()
        for j in inter:
            feature_inter.append(feature_names[j])
        print("interaction {}:".format(i+1), feature_inter, " inter_strength:", round(strength,decimal))
